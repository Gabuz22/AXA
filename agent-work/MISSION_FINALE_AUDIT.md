...
- **Phase 12 — Routage & coût** : ✅ (existant vérifié) `allow_paid_usage=false`, 0 provider payant/carte, chaîne de secours ordonnée par score appris + priorité (`provider_router`), `CostPolicy` + `CostLedger` hebdo. Réserve modèles selon `reasoning_min` (agent_capabilities).
- **Phase 13 — Routines** : ✅ (existant vérifié) orchestrateur central (6 h) + workflows par agent (12 h, sources/quality/…), `concurrency: agents-proposals` (jamais deux en parallèle), timeout, gate `AGENTS_ENABLED`. Schedules non ronds, sans conflit.
- **Phase 17 — Critères de version** : ✅ vérifiés (`MISSION_FINALE_RAPPORT.md`), jalon **v2.9.0**. 210 tests verts, produit intact. Réel : L1/L2/L3-environnement + tous les artefacts déterministes. Simulé (étiqueté) : L3-internes/L4. En attente : run LLM prod (clé), réseau officiel, revue humaine.
