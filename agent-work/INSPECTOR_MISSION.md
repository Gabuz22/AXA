...
- **Phase 14 — Démonstration assistée** : ✅ cas client synthétique traité de bout en bout sur le graphe AXA réel (décomposition → scénarios → arbitrage, validation requise). Constat honnête : matching déterministe à recall large / précision limitée → précision sémantique = IA consommatrice.
- **Phase 15 — Campagne API réelle** : ⏳ NON exécutée (pas de clé LLM ; aucune dépense). À lancer via `agents-knowledge-builder` + `GEMINI_API_KEY`.
- **Phase 19 — Vue IA produit** : ⏳ la projection Inspecteur + `GUIDE_IA.md` + `tools.json` sont prêts (artefact dérivé isolé) ; exposition dans `/ia` = validation humaine requise (non faite, règle 6).
- **Phases 20-21 — Métriques & routine** : ✅ score banc 0.97 ; `inspector-evaluator` ajouté à `DEFAULT_DETERMINISTIC` (suivi de score autonome chaque cycle). Rapport final : `INSPECTOR_RAPPORT.md`. Jalon **v3.0.0**.
